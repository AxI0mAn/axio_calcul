...
export function evaluateFraction() {
  ...
  try {
    ...
    // 1. Сначала переводим всю строку в чистый текстовый вид (⁴ -> 4)
    let cleanExpr = fromSuperscript(fullExpr);

    // 2. ИЗОЛЯЦИЯ ПОКАЗАТЕЛЯ СТЕПЕНИ ДЛЯ ЯДРА:
    // Если в строке есть знак '^' и за ним идут цифры, знаки деления или скобки,
    // мы принудительно оборачиваем всё, что после '^', в скобки.
    // Пример: "3÷5^2÷3" превратится в "3÷5^(2÷3)"
    cleanExpr = cleanExpr.replace(/\^([\d÷().]+)/g, '(^$1)').replace(/\(\^/g, '^(');
    // (Второй replace просто аккуратно переносит скобку: ^(...))

    // 3. Восстанавливаем изоляцию основания дроби, если перед степенью нет скобок
    // Пример: "3÷5^(2÷3)" -> "3÷(5^(2÷3))"
    cleanExpr = cleanExpr.replace(/(\d+)÷(\d+)\^/g, '$1÷($2^');

    console.log("Строка, отправляемая в ядро (ФИКС):", cleanExpr);
