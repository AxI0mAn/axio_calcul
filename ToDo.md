...
если возможна установка приложения:
# внутри nav  - появляется раздел [install]  -> переход на страницу install
# для страниц в папке (home) - появляется кнопка как строка под header download and install this app -> переход на страницу install
# для страниц в папке HE (home) - появляется небольшая кнопка [install]  -> переход на страницу install
страница install: 
1. если это iOS (Safari) - то показать инструкцию как установить (Пользователи iPhone могут установить PWA только вручную через кнопку «Поделиться» (Share) -> «На экран "Домой"» (Add to Home Screen).
